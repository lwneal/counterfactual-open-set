...
python src/train_gan.py --epochs $GAN_EPOCHS >> stdout.txt 2>> stdout.txt

# Baseline: Evaluate the regular classifier
python src/evaluate_classifier.py --result_dir . --mode baseline >> stdout.txt 2>> stdout.txt

# Generate a number of counterfactuals, in K+2 by K+2 square grids
python src/generate_counterfactual.py --result_dir . --count $CF_COUNT >> stdout.txt 2>> stdout.txt

# Automatically label the rightmost column in each grid (ignore the others)
# TODO: Something more elegant than this?
python src/auto_label.py --output_filename generated_images.dataset >> stdout.txt 2>> stdout.txt


# Train a new classifier, now using the aux_dataset containing the counterfactuals
python src/train_classifier.py --epochs $CLASSIFIER_EPOCHS --aux_dataset generated_images.dataset >> stdout.txt 2>> stdout.txt


# Evaluate it one more time just for good measure
python src/evaluate_classifier.py --result_dir . --comparison_dataset /mnt/data/svhn-59.dataset >> stdout.txt 2>> stdout.txt
